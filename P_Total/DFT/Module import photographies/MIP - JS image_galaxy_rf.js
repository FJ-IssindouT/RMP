...
function galaxy_rf()
{
    ...
    for (i=0; i<variables.length; i++) {
        if (variables[i].includes("variable_input_galaxy_rf")) {
            a.push(variables[i]);
        }
        if (variables[i].includes("variable_large_galaxy_rf")) {
            b.push(variables[i]);
        }
    }
    ...
    for (i=0; i<b.length && i<a.length; i++) {
        var dispositif = "RF - Numéro du dispositif : " + RMPApplication.get(a[i]);
    ...
    RMPApplication.set("images_galaxy_rf", c);
